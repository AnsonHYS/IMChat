...
var URL_APIS = {
  BACK: wyyUrl + "/Wylm_wyy/wy/doc/to_myconsult",//返回
  TO_HEALTHY: function (rid) {//医生跳转健康档案
    //busType（业务类型：3门诊；12咨询）
    return baseUrl + '/Platform_N/webHis/treatrecord/doctorHealthyPart?rid=' + rid + "&busType=12"

  },
  INFO: wyyUrl + "/Wylm_wyy/wy/doc/docViewConsultInfo",//查询患者基本资料
...
export default {
  ...
  methods: {
    ...
    toHealthyFile () {
      ...
      window.open(URL_APIS.TO_HEALTHY(this.GroupID), '_blank');
      ...
      this.$axios({
        ...
        if (data.data.resultCode == '200') {
          ...
          if (this.info.status == '4') {
            ...
            this.$store.commit('SET_CHAT_STATUS', true)
          }


          this.$store.commit('SET_DoctorID', this.info.doctorId)//设置医生id
          //生成医生首页地址url
          var obj = {
            'doctorId': this.info.doctorId, 'role': this.role
          }
          this.$store.dispatch("getDocHomePage", obj);
          //设置问答集锦状态
          if (this.info.faqState == '1') {
            this.$store.commit('SET_FAQ_STATUS', true)
          }
          ...
          switch (data.data.resultCode) {
            case "400":
              ...
              let msg = data.data.resultDesc ? data.data.resultDesc : "初始化数据失败！"
              t.$message.error(msg);
